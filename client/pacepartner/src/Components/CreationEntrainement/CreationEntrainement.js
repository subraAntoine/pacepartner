...
export default function CreationEntrainement({
  ...
  const handleCreateEntrainement = async (e) => {
    e.preventDefault();
    try {
      axios.post(`${config.apiUrl}/entrainements/create`, dataEntrainement, {
  ...
  return (
    <div className="creation-entrainement-wrapper">
      ...
      {error && (
        <p className={"crea-entrainement-alert"}>
          {" "}
          Une erreur est survenue lors de la création de l'entraînement.{" "}
        </p>
      )}
      <form
        onSubmit={handleCreateEntrainement}
        className="creation-entrainement-form"
      >
        <div className="item-creation-entrainement">
          <label
            className={"crea-entrainement-label"}
            htmlFor="dateEntrainement"
          >
            {" "}
          ...
          <input
            onChange={handleData}
            className={"crea-entrainement-input"}
            type="date"
            name="dateEntrainement"
            id="dateEntrainement"
            required
          />
          {alertFormat.dateEntrainement && (
            <p className={"crea-entrainement-alert"}>
              {" "}
              La date de l'entraînement ne peut pas être antérieure à la date du
              jour{" "}
            </p>
          )}
        </div>
        <div className="item-creation-entrainement">
          ...
          <input
            ...
            className={"crea-entrainement-input"}
            type="time"
            name="heureEntrainement"
            id="heureEntrainement"
            required
          />
        </div>
        <div className="item-creation-entrainement">
          ...
          <MapBox
            type={"address"}
            onSuggestionSelected={handleAdresseData}
            mapPlaceholder={"Rechercher une adresse"}
          ></MapBox>
        </div>
        <label
          className={"crea-entrainement-label"}
          htmlFor="sportEntrainement"
        >
          {" "}
          Sport{" "}
        </label>
        <div className="item-creation-entrainement-checkbox">
          <div className="checkbox-item-crea-entrainement">
            <input
              onChange={handleData}
              className={"crea-entrainement-input-checkbox"}
              type="radio"
              name="sportEntrainement"
              id="sportEntrainement"
              value="running"
              required
            />
            <label
        ...
        <div className="item-creation-entrainement">
          <label
            className={"crea-entrainement-label"}
            htmlFor="descriptionEntrainement"
          >
            {" "}
          ...
          <textarea
            onChange={handleData}
            className={"crea-entrainement-input"}
            name="descriptionEntrainement"
            id="descriptionEntrainement"
            placeholder={
              "Détaillez les allures et durées des différents intervalles de l'entrainement. "
            }
            required
          />
        </div>
        <div className="item-creation-entrainement">
          ...
          <input
            ...
            className={"crea-entrainement-input"}
            type="number"
            name="distanceEntrainement"
            id="distanceEntrainement"
            required
          />
        </div>
        <div className="item-creation-entrainement">
          ...
          <input
            ...
            className={"crea-entrainement-input"}
            type="number"
            name="dureeEntrainement"
            id="dureeEntrainement"
            required
          />
        </div>
        <div className="item-creation-entrainement">
          ...
          <input
            ...
            required
          />
          {alertFormat.nbMaxParticipants && (
            <p className={"alert-crea-entrainement"}>
              Le nombre de participants doit être compris entre 1 et 8.
            </p>
          )}
        </div>
        <div className="item-creation-entrainement">
          <Button
            className={"crea-entrainement-btn"}
            text={"Créer un entrainement"}
            disabled={
              alertFormat.dateEntrainement || alertFormat.nbMaxParticipants
            }
          ></Button>
        </div>
      </form>
